...
- [x] 7.1 Update `ErrandClient.newTask()` to accept optional `env` parameter — passes `PAPERCLIP_API_KEY`, `PAPERCLIP_API_URL`, `PAPERCLIP_AGENT_ID`, `PAPERCLIP_COMPANY_ID`, `PAPERCLIP_RUN_ID` to errand's `new_task` tool for injection into the task-runner container
- [x] 7.2 Implement `ErrandClient.listSkills()` — calls errand's `list_skills` MCP tool, returns JSON array of `{ name, description }` per skill
- [x] 7.3 Implement `ErrandClient.upsertSkill(name, description, instructions, files?)` — calls errand's `upsert_skill` MCP tool (create-or-update by name; accepts `name`, `description`, `instructions`, optional `files` array of `{ path, content }`)
- [x] 7.4 Implement `ErrandClient.deleteSkill(name)` — calls errand's `delete_skill` MCP tool (delete by name, errors if not found)
- [x] 7.5 Implement `listSkills(ctx)` — uses `resolvePaperclipDesiredSkillNames()` from adapter-utils to read saved skill preferences from `config.paperclipSkillSync.desiredSkills`; maps state as `installed` (desired + in errand), `missing` (desired + not in errand), or `available` (not desired) on the adapter module — calls `ErrandClient.listSkills()`, maps errand skills to `AdapterSkillSnapshot` with `AdapterSkillEntry` objects, cross-references with Paperclip's `config.paperclipRuntimeSkills` to determine desired/managed/state
- [x] 7.6 Implement `syncSkills(ctx, desiredSkills)` — reads skill content from disk via `source` directory path (SKILL.md for instructions + frontmatter description, reference files as `files` array); upserts desired, deletes removed on the adapter module — diffs desired Paperclip skills against errand's current skills, calls `upsertSkill` for new/updated skills and `deleteSkill` for removed skills, returns updated `AdapterSkillSnapshot`
- [x] 7.7 Update `execute()` to pass Paperclip credentials via `env` parameter on `newTask()` — inject `authToken` as `PAPERCLIP_API_KEY`, derive `PAPERCLIP_API_URL` from Paperclip server config, include `PAPERCLIP_AGENT_ID`, `PAPERCLIP_COMPANY_ID`, `PAPERCLIP_RUN_ID` from execution context
- [x] 7.8 Test `ErrandClient` skill CRUD methods (listSkills, upsertSkill, deleteSkill)

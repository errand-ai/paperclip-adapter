...
- [x] 7.5 Implement `listSkills(ctx)` on the adapter module — calls `ErrandClient.listSkills()`, maps errand skills to `AdapterSkillSnapshot` with `AdapterSkillEntry` objects, cross-references with Paperclip's `config.paperclipRuntimeSkills` to determine desired/managed/state
- [x] 7.6 Implement `syncSkills(ctx, desiredSkills)` on the adapter module — diffs desired Paperclip skills against errand's current skills, calls `upsertSkill` for new/updated skills and `deleteSkill` for removed skills, returns updated `AdapterSkillSnapshot`
